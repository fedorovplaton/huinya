...
static char * test_solve_cubic_eqation()
{
    ...
    //printf(" \n %lf %lf %lf %lf \n", get_x0(sPoint), get_x1(sPoint),get_x2(sPoint),get_x3(sPoint));
    mu_assert(2,solve_cubic_equation(1.0, 2.0, 1.0, 1.0, sPoint));
    mu_assert(4,solve_cubic_equation(1.0, -2.0, -1.0, 1.0, sPoint));
    mu_assert(1,solve_cubic_equation(1.0, 4.0, 6.0, 3.0, sPoint));  // (x+1)(x^2 + 3x + 3)
    printf(" \n %lf %lf %lf %lf \n", get_x0(sPoint), get_x1(sPoint),get_x2(sPoint),get_x3(sPoint));
    mu_assert(3,solve_cubic_equation(1.0, 99999.0, -100000.0, 0.0, sPoint)); //x(x+100000)(x-1)
    mu_assert(2,solve_cubic_equation(1.0, -99999.0, -100000.0, 0.0, sPoint));
    mu_assert(2,solve_cubic_equation(1.0, -3.0, 3.0, -1.0, sPoint));
    mu_assert(4,solve_cubic_equation(-1.0, -65.0, 3333.0, 3267.0, sPoint));
    mu_assert(3,solve_cubic_equation(1.0, -3.0, 2.0, 0.0, sPoint));
    mu_assert(2,solve_cubic_equation(1.0, 4.0, 5.0, 2.0, sPoint));  // (x^2 + 2x + 1)(x + 2)
    mu_assert(1,solve_cubic_equation(1.0, 3.0, 3.0, 1.0, sPoint));  // (x^2 + 2x + 1)(x + 1)
    mu_assert(1,solve_cubic_equation(1.0, 3.0, 3.0, 1.0, sPoint));  // (x^2 + 2x + 1)(x + 1)
    mu_assert(1,solve_cubic_equation(1.0, 2.0, 1.0, 0.0, sPoint));  // (x^2 + 2x + 1)(x)
    mu_assert(1,solve_cubic_equation(1.0, 0.0, 0.0, 0.0, sPoint));  //  x^3
